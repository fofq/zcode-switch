...
- **Auto-switch on low quota**: when the active account drops below the threshold (15% by default, configurable), switch to the account with the most quota. The decision is **dual-triggered**: the remaining percentage crossing the threshold **or** the predicted **time-to-empty (ETA)** (remaining tokens ÷ measured burn rate) falling under a safety margin — so it acts minutes before the pool actually dries up. Stale data triggers a refresh first, the chosen target is re-validated right before switching, and when no account is above the threshold it degrades to the best account that still has quota instead of giving up. Every decision is audited (reason / target / latency / quota before-after) to the backend log. An optional **focus model** (e.g. GLM-5.3-Flash) makes both the health check and the switch decision look only at that model's quota; one-click toggle in the toolbar, pairs with "hot switch" while ZCode is running
- **Second-level quota signals, no extra polling**: auto-switch reads the balance snapshots ZCode itself writes into `~/.zcode/v2/logs/` (exact per-pool token amounts, the client's own "plan available" verdict, and the model currently in use) — **zero extra requests, no rate-limit risk**. It automatically falls back to official API polling when the log is unreadable, and Settings shows which source is live and how old the data is
