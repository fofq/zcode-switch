...
const ZH: &[(&str, &str)] = &[
    ...
    ("err.group.too_long", "分组名过长（最多 40 字符）"),
    ...
    ("err.oauth.browser_open", "无法调起系统默认浏览器：{e}"),
    ("err.open.link", "无法打开链接：{e}"),
    ...
    ("err.quota.blocked", "服务端限流（短时间查询过多），稍后会自动重试"),
    ...
    ("err.usage.db", "读取客户端用量数据库失败：{e}"),
    ("err.token.expired", "该账号 Token 已过期，请删除后重新登录"),
    ("err.key.no_access_token", "该账号缺少可用的 access_token，无法铸造 API Key，请重新登录一次"),
    ...
    ("cli.missing_cmd", "缺少子命令：state|list|capture|rename|delete|update|switch|quota|twoapi-test|mint-key|claim-preview|kill|export|export-all|import|behavior|setpath|launch"),
    ("cli.usage.rename", "用法：rename --id <id> --name <名称>"),
    ("cli.usage.mint_key", "用法：mint-key --id <账号id>（铸/取该账号自己的平台 API Key 并写入快照）"),
...
const EN: &[(&str, &str)] = &[
    ...
    ("err.group.too_long", "Group name too long (max 40 characters)"),
    ...
    ("err.oauth.browser_open", "Failed to open the system default browser: {e}"),
    ("err.open.link", "Failed to open the link: {e}"),
    ...
    ("err.quota.blocked", "Server rate limit (too many lookups in a short window) — will retry automatically"),
    ...
    ("err.usage.db", "Failed to read the client usage database: {e}"),
    ("err.token.expired", "This account's token has expired — delete it and log in again"),
    ("err.key.no_access_token", "This account has no usable access_token, so no API key can be minted — log in again"),
    ...
    ("cli.missing_cmd", "Missing subcommand: state|list|capture|rename|delete|update|switch|quota|twoapi-test|mint-key|claim-preview|kill|export|export-all|import|behavior|setpath|launch"),
    ("cli.usage.rename", "Usage: rename --id <id> --name <name>"),
    ("cli.usage.mint_key", "Usage: mint-key --id <account id> (mints/fetches the account's own platform API key and saves it to the snapshot)"),
